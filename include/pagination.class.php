...
class Pagination 
{
	...
	function create_links()
	{
		...
		$output = null;
		$output = '<li class="page_info">Page ('.$currentPage.'/'.$totalPages.') </li>';
		...
		if ($loopStart != 1){
			$output .= sprintf('<li class="nextpage"><a href="' . $link . '"> &#171; </a></li>', '1');
		}
		...
		{
			if ($i == $currentPage){
				$output .= '<li class="active_page"><a href="'.$_SERVER['REQUEST_URI'].'">' . $i . '</a></li> ';
			} else {
		...
		return '<div class="page_numbers"><ul>' . $output . '</ul></div>';
	}
}
