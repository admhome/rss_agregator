<?php
	...
	$smarty->assign('category_id',$row['category_id']); // assign the category id
	$smarty->assign('category_title',$row['category_title']); // assign the category title
	// number of feed items categorized in this category
	$items_number = mysql_num_rows(mysql_query("SELECT item_category_id FROM feeditems WHERE item_category_id='$id' AND item_published='1'"));
	...
	if ($friendly_urls == 1) {
	$pagination->setLink("category-$id-%s-".url_slug($row['category_title'])); // set the link of the pages
	} else {
	$pagination->setLink("category.php?id=$id&page=%s"); // set the link of the pages
	}
	...
	WHERE feeditems.item_category_id='$id' AND feeditems.item_published='1'
	...
	$smarty->assign('page_title',$seo_title.' | '.$row['category_title']); // title of the page
	$smarty->display('category.html'); // display all the assigned data in a template 'category.html'
?>