<?php
	...
	$smarty->assign('new_items_number',$new_items_number); // number of items in each page
	...
	$items_number = mysql_num_rows(mysql_query("SELECT item_unix_datetime FROM feeditems WHERE item_published='1' AND item_unix_datetime BETWEEN $unix_date AND $to_unix_date"));
	$smarty->assign('items_number',$items_number);
	if ($items_number > 0) { // if feed items number is more than 0
	$page = 1;
	$size = $new_items_number; // extracted from include/setting.php
	if (isset($_GET['page'])){ $page = abs(intval($_GET['page'])); }
	// define the pagination class (include/pagination.class.php)
	$pagination = new Pagination();
	if ($friendly_urls == 1) {
	$pagination->setLink("date-$date"."_page%s"); // set the link of the pages
	} else {
	$pagination->setLink("items_by_date.php?date=$date&page=%s"); // set the link of the pages
	}
	$pagination->setPage($page);
	$pagination->setSize($size);
	$pagination->setTotalRecords($items_number);
	// feed items in this date
	$category_sql = "SELECT 
	feeditems.item_id,
	...
	$smarty->assign('page_title',$seo_title.' | Items Published On : '.$date); // title of the page
	$smarty->display('items_by_date.html'); // display all the assigned data in a template 'items_by_date.html'
?>