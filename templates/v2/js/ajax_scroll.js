(function($) {
	...
	$.fn.scrollPagination = function(options) {
		...
		return this.each(function() {		
			...
			if($settings.scroll == true) $initmessage = 'No More Results';
			else $initmessage = 'More Results';
			...
			function getData() {
				...
				$.post($settings.ajax_file, {
					...
					if(data == "") { 
						$('.loading-bar').html('No More Results');	
					}
					else {
						// Offset increases
			...
			if($settings.scroll == true) {
				...
				$(window).scroll(function() {
					...
					if($(window).scrollTop() + $(window).height() > $this.height() && !busy) {
						...
						$this.find('.loading-bar').html('More Results');
